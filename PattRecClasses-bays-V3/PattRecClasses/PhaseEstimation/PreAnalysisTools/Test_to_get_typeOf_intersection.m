...
hmm_adaptive = make_initial_HMM_from_DirichletParameters...
   ...
   emissionProbs = fixedTimeRandomEmissions();
...
numTrained = 5;%floor(numTests/2);
numclassified = 5;% ceil(numTests/2);
...
figure 
plot(-logP_fix(1,1:numTrained),-logP_fix(2,1:numTrained), 'r+','markersize',5)
 hold on
...
ylabel( '$-\ln\left(\Pr\left(o|\lambda_{s}\right)\right)$', 'Interpreter', 'latex' );
set(0,'DefaultAxesFontSize',12);
plot(-logP_fix(1,numTrained+1:end),-logP_fix(2,numTrained+1:end), 'g^','markersize',8, 'linewidth',1)
